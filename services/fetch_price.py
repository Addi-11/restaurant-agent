...
class FetchPriceService:
    ...
    def extract_dish_query(user_message: str) -> Optional[PriceQuery]:
        ...
        try:
            ...
            logger.error(f"Failed to parse dish price query: {e}\nModel output: {model_output}")
            return None
    
    def process_request(self, user_message: str) -> FetchPriceResponse:
        query = self.extract_dish_query(user_message)
        if not query:
            return FetchPriceResponse(dish_name="", message="Could not extract dish details from query.")
        ...
        if not found:
            return FetchPriceResponse(dish_name=query.dish_name, message="Dish not found in any restaurant.")
