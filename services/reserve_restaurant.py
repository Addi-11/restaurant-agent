...
class ReserveRestaurantService:
    ...
    def extract_reservation_details(user_message: str) -> Optional[ReservationQuery]:
        ...
        try:
            ...
            logger.error(f"Failed to parse reservation details: {e}\nModel output: {model_output}")
            return None
    ...
    def process_request(self, user_message: str) -> ReservationResponse:
        ...
        if not details:
            return ReservationResponse(message="Could not extract reservation details. Please provide restaurant name, number of people, and date-time.")
        ...
        if missing_fields:
            return ReservationResponse(message=f"Please provide the {', '.join(missing_fields)} to proceed with the reservation.")

        try:
            booking_time = datetime.strptime(details.date_time, "%Y-%m-%d %H:%M").strftime("%Y-%m-%d %H:%M")
        except ValueError:
            return ReservationResponse(message="Invalid date format. Please use YYYY-MM-DD HH:MM.")

        return self.reserve_table(details.restaurant_name, details.num_people, booking_time)
